...
db.events.find({replaced_timestamp: "NULL"}).forEach(function(e){e.replaced_timestamp = null; db.events.save(e);})
db.events.find({deployer: "NULL"}).forEach(function(e){e.deployer = "n/a"; db.events.save(e);})

print("la alle timestamps være på isodate formatet")
db.events.find().forEach(function(a){a.deployed_timestamp = ISODate(a.deployed_timestamp); db.events.save(a)});
db.events.find({replaced_timestamp:{ $ne: ""}}).forEach(function(a){a.replaced_timestamp = ISODate(a.replaced_timestamp); db.events.save(a)});
