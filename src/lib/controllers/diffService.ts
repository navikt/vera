...
import { IEvent, IEnvironment } from "@/interfaces/IEvent";
...
export async function diffEnvironments (base:string, comparewith:string) {
    ...
    const events = await Event.find({replaced_timestamp: {$ne: null},version: {$ne: null},environment: {$in: [base, comparewith]}});
    //console.log(events)
