...
interface IGroupBy { [key: string]: IEventEnriched[] }
...
function buildVersionMatrixTable(tableHeader: IHeader[], applicationInstances: IGroupBy, columnKey: string): IFilteredJsonData {
  ...
  const tableContents = rowKeys.map((element) => generateRow(tableHeader, element, applicationInstances[element], columnKey));
...
function getTableHeader(versionData: IEventEnriched[], key: keyof IEventEnriched): IHeader[] {
  const headerData = [
    ...new Set(versionData.map((item) => item[key] as string))
  ]
...
function generateRow(columns: IHeader[], firstColumnTitle: string, applicationInstances: IEventEnriched[], elementKey: string) {
  ...
  for (let i = 1; i < columns.length; i++) {
    const filtered: IEventEnriched[] = applicationInstances.filter(appInstance => appInstance[elementKey] === columns[i].columnTitle);
